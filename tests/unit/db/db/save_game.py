...
@pytest.mark.asyncio
async def test_round_insert_uses_uppercase_country_code(mock_db):
    # Arrange
    adapter, mock_conn = mock_db
    ...
    game = {
        ...
        "map_name": "World",
        "is_daily": False,
        "played_at": "2024-06-01T10:00:00Z",
        "rounds": [
            {
                "round_number": 1,
                "score": 5000,
    ...
    with (
        patch("db.db._resolve_state_id", return_value=None),
        patch("db.db._resolve_ecoregion", return_value=(None, None)),
    ):
        # Act
        await adapter.save_game(game)

    # Assert — round was inserted
    assert mock_conn.execute.call_count == 2
