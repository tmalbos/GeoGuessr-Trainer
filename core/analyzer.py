from core.geo_enrich import enrich_parallel
from db.mongo import save_game
from anki.generator import generate_cards_for_game
...
def parse_and_display(game_data: dict, game_id: str):
    ...
    for i, (rnd, guess) in enumerate(zip(rounds_raw, guesses), start=1):
        score_pts = guess.get("roundScoreInPoints", 0)
        dist_km   = round(guess["distanceInMeters"] / 1000, 1) if guess.get("distanceInMeters") else None
        ...
        print(f"  📏 Distancia   : {dist_km:,} km" if dist_km is not None else "  📏 Distancia   : —")
        print(f"  👣 Pasos       : {steps}")
